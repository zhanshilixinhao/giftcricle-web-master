<template>
  <div class="app-container" v-loading="loading2"
    ...
    <div>
      <el-card :body-style="{ padding: '12px' }">
        ...
        <el-form :model="ruleForm" :label-position="left" ref="ruleForm" label-width="100px">
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="文章节日标题" prop="title">
                <el-input v-model="ruleForm.title" placeholder="文章节日标题" type="text"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <!--文章节日-->
    <el-dialog title="文章节日" :visible.sync="dialogVisible" size="tiny">
      <el-form :model="addForm" :rules="rules" ref="addForm" label-width="100px">
        <el-form-item label="节日名称" prop="title">
          <el-input v-model="addForm.title" placeholder="节日名称" type="text"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import { getArticleFestival, addArFestival, updateArFestival, delArFestival } from '../../api/gift/article/festival'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        this.loading2 = true
        getArticleFestival(search).then(re =>{
          this.loading2 = false
      ...
      addInfo() {
        addArFestival({ title: this.addForm.title }).then(re => {
      ...
      updateInfo() {
        updateArFestival({ id: this.id, title: this.addForm.title }).then(re => {
          if (re.data.errCode === 0){
            this.$message({
              type: 'success',
              message: '修改成功'
      ...
      handleDelete(index, row) {
        this.$confirm('你确定要删除吗?删除后不可恢复')
          .then(_ => {
            this.loading2 = true
            delArFestival({ id: row.id }).then(re => {
              this.loading2 = false
              if (re.data.errCode === 0) {
                this.$message({
                  message: re.data.msg,
