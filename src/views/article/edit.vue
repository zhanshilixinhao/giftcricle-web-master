...
<script>
  ...
  import { updateArticleApi, getArticleDetail, getSceneListAll, getFestivalListAll , getArLabelListAll} from '../../api/gift/article/article'
  ...
  export default {
    ...
    methods: {
      getDetail() {
        getArticleDetail(this.$route.params.id).then(re => {
          if (re.data.errCode !== 0) {
            this.$router.push('/banner/list')
            this.$message.error('文章信息有误!')
          }
          ...
          this.ruleForm.scene = data.sceneId
          this.ruleForm.label = data.labelId
          this.ruleForm.festival = data.festivalId
          // this.ruleForm.showTime = data.showTime
      ...
      updateArticle() {
        ...
        // article.showTime = moment(this.ruleForm.showTime).format('YYYY/MM/DD HH:mm:ss')

        // 添加文章
        this.loading2 = true
        updateArticleApi(article).then(re => {
