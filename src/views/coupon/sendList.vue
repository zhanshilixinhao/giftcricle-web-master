...
<script>
  ...
  import { getSendRecordList, deleteSendRecord } from '../../api/gift/coupon/coupon'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        var adminId = 1
        this.loading2 = true
        getSendRecordList({pageNum:this.pageNum, pageSize:this.pageSize, title:title}).then(re => {
          this.loading2 = false
      ...
      handleDelete(index, row) {
        this.$confirm('你确定要删除该记录吗?删除后无法恢复!')
          .then(_ => {
            this.loading2 = true
            deleteSendRecord({sendId:row.id}).then(re => {
              this.loading2 = false
