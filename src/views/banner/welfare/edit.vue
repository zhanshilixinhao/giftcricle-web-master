<template>
  <div v-loading="loading2"
    ...
    <div class="app-container">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
        ...
        <el-row :gutter="12">
          ...
          <el-col :span="2">
            <el-form-item>
              <el-button v-if="ruleForm.type&&ruleForm.type!==''" type="primary" @click="addItem()" size="small">选择物品
              </el-button>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <!--商品选择-->
    <el-dialog :title="type" :visible.sync="dialogVisibleItem" size="tiny" center>
    ...
    <!--虚拟商品和优惠券选择-->
    <el-dialog :title="type" :visible.sync="dialogVisible" size="tiny" center>
    ...
    <!--sku列表-->
    <el-dialog :title="type" :visible.sync="dialogVisibleSku" size="tiny" center>
      <el-table
        :data="tableData6"
        stripe
        border>
...
<script>
  import { updateWelfare, detailWelfare, getAllItemList, getSkuList, itemDetail } from '../../../api/home/banner/welfare'
  ...
  export default {
    ...
    methods: {
      ...
      // 详情
      getDetail() {
        detailWelfare(this.$route.params.id).then(re => {
          ...
          this.ruleForm.isCode = data.isCode + ''
          this.ruleForm.targetDate = moment(data.targetDate)
          this.ruleForm.quantity = data.quantity
          this.ruleForm.targetId = data.targetId
          if (data.type === 1) {
            this.skuId = data.targetId
          } else {
            this.itemId = data.targetId
          }
          this.ruleForm.value4 = [moment(data.startTime), moment(data.endTime)]
          // this.$refs.pick.pick([data.startTime, data.endTime])
          console.log(this.ruleForm)
          if (this.ruleForm.type === 1) {
            this.ruleForm.targetId = this.skuId
          } else {
            this.ruleForm.targetId = this.itemId
          }
          itemDetail({ type: data.type, id: data.targetId}).then(re => {
            this.ruleForm.detail = re.data.data
          })
        })
      },
