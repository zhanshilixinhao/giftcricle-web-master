<template>
  <div class="app-container" v-loading="loading2"
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="标签名称" prop="name">
                <el-input v-model="ruleForm.name" placeholder="标签名称" type="text" ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <el-table
      ...
      <el-table-column width="100" label="标签封面" align="center">
    ...
    <el-dialog title="添加标签" :visible.sync="dialogVisible" size="tiny"  :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="100px">
        <el-form-item label="标签名称">
          <el-input  v-model="addForm.name">
          </el-input>
        </el-form-item>
        <el-form-item label="标签封面" prop="imageUriList">
          <el-upload
            :action="uploadUrl"
    ...
    <el-dialog title="修改标签" :visible.sync="dialogUpdateVisible" size="tiny"  :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false">
...
<script>
  ...
  import { getLabelList, deleteLabel, addLabel, updateLabel, updateStatus } from '@/api/gift/labelItem/label'
  ...
  export default {
    ...
    methods: {
      ...
      addSubmit() {
        ...
        addLabel(this.addForm.name, this.addForm.imageUriList[0]).then(re => {
          if (re.data.errCode === 0) {
      ...
      getList() {
        ...
        this.loading2 = true
        getLabelList(this.pageNum, this.pageSize, name).then(re => {
          this.loading2 = false
      ...
      handleDelete(index, row) {
        this.$confirm('你确定要删除该标签以及该标签的标签商品吗?')
          .then(_ => {
            this.loading2 = true
            deleteLabel(row.id).then(re => {
              this.loading2 = false
              if (re.data.errCode === 0) {
                this.$message({
                  message: re.data.msg,
