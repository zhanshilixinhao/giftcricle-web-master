<template>
  <div class="app-container" v-loading="loading2"
      ...
      <el-card :body-style="{ padding: '12px' }">
        ...
        <el-form :model="ruleForm" :label-position="left" ref="ruleForm" label-width="100px">
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              <el-form-item label="标签商品状态" prop="status">
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="标签选择" prop="labelId">
                <el-select v-model="ruleForm.labelId" filterable placeholder="请选择" style="width: 100%" icon="el-icon-search" @change="categoryChange">
                  <el-option
    ...
    <div>
      <el-card :body-style="{ padding: '12px' }">
        <el-button type="primary" @click="addTheme()">添加标签商品</el-button>
      </el-card>
    </div>
    <br/>
    <el-table
    ...
    <el-dialog title="标签选择" :visible.sync="themeVisible">
      <el-select v-model="themes" filterable placeholder="请先选择标签" style="width: 100%" icon="el-icon-search" @change="themeChange">
        <el-option
    ...
    <el-dialog :title="themeName" :visible.sync="dialogVisible" size="tiny"  :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false" center>
...
<script>
  ...
  import { getAllList } from '@/api/gift/labelItem/label'
  import { getLabelItemList, delLabelItem, addLabelItem, changeStatus, setLabelItemSort } from '@/api/gift/labelItem/labelItem'
  ...
  export default {
    ...
    methods: {
      ...
      addSubmit() {
        ...
        addLabelItem(this.themes, ids).then(re => {
          if (re.data.errCode === 0) {
      ...
      updateSubmit() {
        ...
        setLabelItemSort(theme).then(re => {
          if (re.data.errCode === 0) {
            this.$message.success(re.data.msg)
            this.dialogUpdateVisible = false
            this.updateForm.id = null
      ...
      getList() {
        ...
        search.labelId = this.ruleForm.labelId
        this.loading2 = true
        getLabelItemList(this.pageNum, this.pageSize, search).then(re => {
          this.loading2 = false
      ...
      handleDelete(index, row) {
        this.$confirm('你确定要删除该标签商品吗?删除后无法恢复!')
          .then(_ => {
            this.loading2 = true
            delLabelItem(row.id).then(re => {
              this.loading2 = false
              if (re.data.errCode === 0) {
                this.$message({
                  message: re.data.msg,
