<template>
  <div class="app-container" v-loading="loading2"
    ...
    <el-table
      ...
      <el-table-column width="100%" label="封面图片" align="center">
...
<script>
  ...
  import { getVirList, delVirItemOrder } from '@/api/order/virItemOrder'
  ...
  export default {
    ...
    mounted() {
      this.getList()
      this.$store.getters.sidebar.viItemCount = 0
    },
    methods: {
      // // 添加用户
      // add() {
      ...
      handleDelete(index, row) {
        this.$confirm('你确定要删除该订单吗?')
          .then(_ => {
            this.loading2 = true
            delVirItemOrder(row.id).then(re => {
              this.loading2 = false
              if (re.data.errCode === 0) {
                this.$message({
                  message: re.data.msg,
