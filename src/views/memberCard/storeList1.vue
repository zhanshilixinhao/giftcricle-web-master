<template>
  <div class="app-container" v-loading="loading2" element-loading-text="拼命加载中">
    ...
    <br/>
    <div v-if="this.$store.state.user.roleId && this.$store.state.user.roleId !== 5">
      <el-card :body-style="{ padding: '12px' }">
    ...
    <el-table
      ...
      <el-table-column label="操作" align="center" width="350px" v-if="this.$store.state.user.roleId && this.$store.state.user.roleId !== 5">
        <template scope="scope">
          ...
          <el-button
            size="mini"
            type="success"
            v-if="!scope.row.adminId && scope.row.id !== 0"
            @click="handlePass(scope.row)">绑定后台
          </el-button>
          <el-button
            size="mini"
            type="info"
            v-if="scope.row.adminId && scope.row.id !== 0">已绑定
          </el-button>
          <el-button
            v-if=" scope.row.id !== 0"
            icon="el-icon-delete"
    ...
    <el-dialog title="门店信息" :visible.sync="dialogVisible" width="50%">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px" class="demo-form">
        ...
        <el-form-item label="门店密码" prop="password">
          <el-input v-model="form.password" placeholder="如果不设置/修改密码可以不用填写"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getList()
      if(this.$store.state.user.roleId && this.$store.state.user.roleId !== 5){
        this.getDistrict()
      }
    },
    methods: {
      ...
      store() {
        ...
        if (this.form.password){
          store.password = new MD5().update(this.form.password).digest('hex').toUpperCase()
        }
